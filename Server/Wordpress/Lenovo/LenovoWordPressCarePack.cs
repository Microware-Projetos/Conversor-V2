...
namespace eCommerce.Server.Wordpress.Lenovo;

public static class LenovoWordPressCarePack
{
    private const string WOOCOMMERCE_BASE_URL = "https://ecommerce.microware.com.br/lenovo/wp-json/wc/v3/products";
    private const string WOOCOMMERCE_CONSUMER_KEY = "ck_27e249ea7ce48002377a8b34e210d56e683ba8a7";
    private const string WOOCOMMERCE_CONSUMER_SECRET = "cs_326d654a9c568295e92714d51e93a4605e2683bf";
    ...
    public static async Task EnviarListaDeCarePack(List<WooProduct> carePacks)
    {
        ...
        try
        {
            Console.WriteLine("[INFO]: Iniciando processo de envio de produtos para o WooCommerce...");
            ...
            await DeletarTodosProdutos(carePacks, todosCarePacks);

            Console.WriteLine("[INFO]: Enviando carepacks para o WooCommerce...");
            await EnviarProdutosEmLotes(carePacks);
            */
        }
        catch(Exception ex)
        {
    ...
    private static async Task<List<WooProduct>> BuscarCarePacks()
    {
        ...
        while(true)
        {
            ...
            var response = await _http.GetAsync($"{WOOCOMMERCE_BASE_URL}?per_page=100&page={page}&category=33"); 
            ...
            foreach(var carePack in carePacks)
            {
                ...
                if(categories == 33)
                {
                    Console.WriteLine($"[INFO]: ID da categoria do carepack: {categories}");
                    Console.WriteLine($"[INFO]: SKU do carepack: {sku}");
