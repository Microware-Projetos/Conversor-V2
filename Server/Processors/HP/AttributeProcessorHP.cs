...
public static class AttributeProcessorHP
{
    public static List<WooAttribute> ProcessarAttributes(string sku, string model, IXLRow linha, IXLRow cabecalho, Dictionary<string, string> prodInfo, string sheetName, Dictionary<string, string> normalizedAnatel, Dictionary<string, string> normalizedFamily)
    {
        ...
        var mapData = JsonConvert.DeserializeObject<dynamic>(File.ReadAllText("Maps/HP/map.json"));
        ...
        var attributesMappingWp = JsonConvert.DeserializeObject<List<AttributeMap>>(File.ReadAllText("Maps/HP/atributes.json")) ?? new List<AttributeMap>();
        ...
        attributes.Add(new WooAttribute { id = 45, options = "HP", visible = true });
